...
def get_data(filters, period_list, partner_doctype):
	...
	for d in sales_users_data:
		...
		if d.item_code:
			sales_user_wise_item_groups[d.parent].append(d.item_code)
		...
		for period in period_list:
			...
			for r in actual_data:
				if (
					...
					and (not sales_user_wise_item_groups.get(d.parent) or r.item_code == d.item_code)
				):
...
def get_actual_data(filters, sales_users_or_territory_data, date_field, sales_field):
	...
	return frappe.db.sql(
		""" SELECT `tab{child_doc}`.item_code,
...
def get_parents_data(filters, partner_doctype):
	...
	return frappe.get_all(
		...
		fields=["parent", "custom_item", target_qty_amt_field, "fiscal_year", "distribution_id"],
	)
